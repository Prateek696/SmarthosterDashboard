/**
 * Server Component - Full Service Management Page Entry Point (Locale-based)
 */
import { strapiApi } from "@/services/strapi.api";
import { mapToStrapiLocale } from "@/utils/strapi-helpers";
import FullServiceManagementPageClient from "../../full-service-management/page-client";
...
export default async function FullServiceManagementPage({ params }: PageProps) {
  ...
  let strapiData = null;
  try {
    strapiData = await strapiApi.getFullServiceManagementPage(strapiLocale, {
      cache: 'no-store'
    });
    ...
    console.log(`📊 Server: Full Service Management Strapi Data (locale: ${locale}):`);
    ...
    console.log('- benefitsList:', strapiData?.benefitsList?.length || 0, 'items');
  ...
  return <FullServiceManagementPageClient strapiData={strapiData} />;
}
