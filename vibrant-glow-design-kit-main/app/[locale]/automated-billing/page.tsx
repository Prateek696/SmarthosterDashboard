/**
 * Server Component - Automated Billing Page Entry Point (Locale-based)
 */
import { strapiApi } from "@/services/strapi.api";
import { mapToStrapiLocale } from "@/utils/strapi-helpers";
import AutomatedBillingPageClient from "../../automated-billing/page-client";
...
export default async function AutomatedBillingPage({ params }: PageProps) {
  ...
  let strapiData = null;
  try {
    strapiData = await strapiApi.getAutomatedBillingPage(strapiLocale, {
      cache: 'no-store'
    });
    ...
    console.log(`📊 Server: Automated Billing Strapi Data (locale: ${locale}):`);
  ...
  return <AutomatedBillingPageClient strapiData={strapiData} />;
}
