/**
 * Server Component - Local Expertise Page Entry Point (Locale-based)
 */
import { strapiApi } from "@/services/strapi.api";
import { mapToStrapiLocale } from "@/utils/strapi-helpers";
import LocalExpertisePageClient from "../../local-expertise/page-client";
...
export default async function LocalExpertisePage({ params }: PageProps) {
  ...
  let strapiData = null;
  try {
    strapiData = await strapiApi.getLocalExpertisePage(strapiLocale, {
      cache: 'no-store'
    });
    ...
    console.log(`📊 Server: Local Expertise Strapi Data (locale: ${locale}):`);
  ...
  return <LocalExpertisePageClient strapiData={strapiData} />;
}
