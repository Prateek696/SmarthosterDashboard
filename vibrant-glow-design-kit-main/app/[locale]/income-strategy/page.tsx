/**
 * Server Component - Income Strategy Page Entry Point (Locale-based)
 */
import { strapiApi } from "@/services/strapi.api";
import { mapToStrapiLocale } from "@/utils/strapi-helpers";
import IncomeStrategyPageClient from "../../income-strategy/page-client";
...
export default async function IncomeStrategyPage({ params }: PageProps) {
  ...
  let strapiData = null;
  try {
    strapiData = await strapiApi.getIncomeStrategyPage(strapiLocale, {
      cache: 'no-store'
    });
    ...
    console.log(`📊 Server: Income Strategy Strapi Data (locale: ${locale}):`);
  ...
  return <IncomeStrategyPageClient strapiData={strapiData} />;
}
