...
interface HomePageClientProps {
  strapiData?: any; // Strapi homepage data
}

export default function HomePageClient({ strapiData }: HomePageClientProps) {
  const { t } = useLanguage();
    ...
    try {
      if (typeof window !== 'undefined' && window.location && window.location.hash) {
        ...
        if (hash.includes('error=access_denied') || hash.includes('error_code=otp_expired')) {
          ...
          router.push(`/auth?error=${error}&error_code=${errorCode}&error_description=${encodeURIComponent(errorDescription || '')}`);
          return;
        }
      }
    } catch (error) {
  ...
  return (
    <Layout customSEO={customSEO}>
      ...
      <Features strapiData={strapiData} />
      <Integrations strapiData={strapiData} />
      <Testimonials strapiData={strapiData} />
      <SuccessStories strapiData={strapiData} />
      <HowItWorks strapiData={strapiData} />
      <FAQ strapiData={strapiData} />
      <CTA strapiData={strapiData} />
      <div id="contact">
        <ContactForm strapiData={strapiData} />
      </div>
    </Layout>
  );
}
